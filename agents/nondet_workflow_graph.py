...
def create_non_deterministic_workflow_graph() -> StateGraph:
    ...
    async def initialize_node(state: WorkflowState) -> Dict[str, Any]:
        """Initialize MCP session and generate run_id early for complete observability"""
        # Generate run_id early so all workflow events can be traced
        run_id = state.get("run_id") or str(uuid.uuid4())
        
        url = state.get("mcp_server_url", "http://localhost:3333/mcp")
        mcp_caller = McpToolCaller(url, "", run_id)
    ...
    async def register_output_node(state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            ...
            if new_run_id:
                ...
                # Log correlation: trace_id (used for logging) vs db_run_id (from database)
                trace_id = run_id  # The UUID we generated early for tracing
                db_run_id = new_run_id  # The UUID generated by PostgreSQL
                
                if trace_id != db_run_id:
                    # Log correlation for both IDs
                    log_trace(trace_id, {
                        "node": "register_output",
                        "event": "completed",
                        "trace_id": trace_id,
                        "db_run_id": db_run_id,
                        "note": "trace_id used for logging, db_run_id from database"
                    })
                    # Also log to db_run_id file for database correlation
                    log_trace(db_run_id, {
                        "node": "register_output",
                        "event": "completed",
                        "trace_id": trace_id,
                        "db_run_id": db_run_id,
                        "note": "correlated with trace_id"
                    })
                else:
